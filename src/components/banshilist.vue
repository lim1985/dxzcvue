...
<div class="tops mg">
    ...
    <div class="loading-layout" v-loading="onloading">
        ...
        <div class="more"><i class="icon iconfont icon-gengduo2"></i></div></header>
 ...
 myBanShidetail
	...
	methods:
	{
     ...
     {  
        ...
        {
               console.log(id)
                    ///'/Category_1189/Index_'+this.nowpageindex+'.aspx'正式接口
           ///detail 测试接口
          this.axios.get('/Category_1189/Index_'+this.nowpageindex+'.aspx', {//获取新闻列表的内容
            params: {
              itemid: id
            }
          }).then(res => {          
        ...
        HideandShowbanshiDetail:function()
        {
           ...
           params: this.myBSparams  
              ...
              {          
                this.onloading=false;
                  if(typeof res.data=='string')
                {
                    this.bsfwlist=(this.Myunilt.fixhtmltoJSON(html));   
                    console.log(this.bsfwlist)
                }
                else
                {
                    this.bsfwlist=res.data.newslist;
                     console.log(this.bsfwlist)
                }            
              }

          }).catch(function () {
            console.log('请求失败了111122！');
          });
